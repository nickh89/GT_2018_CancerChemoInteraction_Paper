...
### read in data
IaData<-read.csv(file="/Users/nickhousley/Desktop/Projects /Papers/GT_2018_Cancer_Chemo_Interaction_Paper/GT_2018_CancerChemoInteraction_Paper/Data/Ia/Ia_Final_a1.csv", header=TRUE, sep=",", check.names = FALSE) # read in data and filter
head(IaData)
...
DataIaRaw<-IaData[,c(2,5,9:25,29:43)]
...
res.man <- aov(DataIaRaw$Dyn.pfr ~ as.factor(treatment), data = DataIaRaw)
summary.aov(res.man)###individual tests
...
DataIaRaw %>%
  group_by(treatment)%>%
  summarise(meanForce=mean(Thr.L))
